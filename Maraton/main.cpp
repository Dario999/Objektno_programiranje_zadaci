...
private:
    ...
    int brUcesnici;
public:
    Maraton(const char* lokacija="",Ucesnik *U=0,int brUcesnici=0) {
        this->brUcesnici=brUcesnici;
        strcpy(this->lokacija,lokacija);
        this->U=new Ucesnik[brUcesnici];
    ...
    Maraton(const Maraton &u) {
        this->brUcesnici=u.brUcesnici;
        strcpy(this->lokacija,u.lokacija);
        this->U=new Ucesnik[u.brUcesnici];
        for(int i=0; i<u.brUcesnici; ++i) {
            this->U[i]=u.U[i];
        }
    }
    ...
    Maraton &operator+=(const Ucesnik &b){
    Ucesnik *temp;
    temp=new Ucesnik[brUcesnici+1];
    for(int i=0;i<brUcesnici;++i){
        temp[i]=U[i];
       }
     delete[] this->U;
    ...
    float vkupnoV=0;
    for(int i=0;i<brUcesnici;++i){
        vkupnoV+=U[i].getVozrast();
            }
      return float(vkupnoV)/brUcesnici;
    }

  void pecatiPomladi(Ucesnik &u){
    for(int i=0;i<brUcesnici;++i){
        if(U[i].getVozrast()<u.getVozrast())
            cout<<U[i]<<endl;
        }
    }
