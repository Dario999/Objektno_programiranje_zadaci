...
private:
    ...
    int brojAkcii;
public:
    StockRecord(const char *ID="",const char *ime="",float cena=0,int brojAkcii=0,float momCena=0){
        strcpy(this->ID,ID);
        strcpy(this->ime,ime);
        this->cena=cena;
    ...
    double value(){
            return  brojAkcii*momCena;
    }

    double profit(){
        return brojAkcii*(momCena - cena);
    }

    friend ostream& operator<<(ostream &out,StockRecord &sr){
        out<<sr.ime<<" "<<sr.brojAkcii<<" "<<sr.cena<<" "<<sr.momCena<<" "<<sr.profit()<<endl;
        return out;
    }
...
private:
    ...
    int brObjekti;
public:
    Client(const char *ime="",int id=0,StockRecord *c=0,int broj=0){
        strcpy(this->ime,ime);
        ID_smetka=id;
        brObjekti=broj;
        this->stockrecord = new StockRecord[brObjekti];
    ...
    Client(const Client &c){
        strcpy(ime,c.ime);
        ID_smetka=c.ID_smetka;
        brObjekti=c.brObjekti;
        stockrecord = new StockRecord[c.brObjekti];
        for(int i=0;i<brObjekti;i++)
            stockrecord[i]=c.stockrecord[i];
    }
    ...
    double totalValue(){
        double sum=0;
        for(int i=0;i<brObjekti;i++)
            sum+=stockrecord[i].value();
        return sum;
    }

    Client &operator+=(const StockRecord& c){
        StockRecord *temp;
        temp = new StockRecord[brObjekti+1];
        for(int i=0;i<brObjekti;i++)
            temp[i]=stockrecord[i];
        delete[] this->stockrecord;
        stockrecord = temp;
        stockrecord[brObjekti++] = c;
        return *this;
    }

    friend ostream &operator<<(ostream &out,Client &c){
        out<<c.ID_smetka<<" "<<c.totalValue()<<endl;
